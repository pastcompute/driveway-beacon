...
bool setupmlx(int addr) {
  Serial.println("setupmlx");
  Wire.beginTransmission(addr);
  Wire.write(0x60);
  Wire.write(0x00); // BIST disabled
  Wire.write(0x5c); // Hall plate spinning rate = 0, GAIN_SEL = 5
  Wire.write(0x00);
  Wire.endTransmission();
  int status = getmlxstatus(addr);
  Serial.print("setupmlx status1="); Serial.println(status);

  Wire.beginTransmission(addr);
  Wire.write(0x60);
  Wire.write(0x02);
  Wire.write(0xb4); // magnetic res = 0
  Wire.write(0x08);
  Wire.endTransmission();
  status = getmlxstatus(addr);
  Serial.print("setupmlx status2="); Serial.println(status);
  delay(300);
  return true;
}

bool getmlx(int addr, int&x, int&y, int&z, int &s)  {
  Serial.println("getmlx");
  Wire.beginTransmission(addr);
  Wire.write(0x3e); // single measure mode
  Wire.endTransmission();
  int status = getmlxstatus(addr);
  Serial.print("getmlx status="); Serial.println(status);
  delay(100);
  Wire.beginTransmission(addr);
  Wire.write(0x4e); // single measure mode
  Wire.endTransmission();
  ...
  if (Wire.available() == 7) {
    ...
    z = data[5] *256 + data[6];
    s = data[0];
    Serial.print("getmlx datastatus="); Serial.println(s);
    return true;
  } else {
...
float maxx=0.F, maxy=0.F, maxz=0.F, maxm = 0.F;
bool domlx = true;

void loop() {
  if (found_mlx && domlx) {
    int x,y,z,m,s=-1;
    bool ok = getmlx(MLX_ADDR, x, y, z, s);
    if (ok) {
    m = sqrt(x*x + y*y + z*z);
    if (fabsf(x) > maxx) { maxx = fabsf(x);}
    if (fabsf(y) > maxy) { maxy = fabsf(y);}
    if (fabsf(z) > maxz) { maxz = fabsf(z);}
    if (fabsf(m) > maxm) { maxm = fabsf(m);}
    Serial.print("uTeslas X="); Serial.print(x);
